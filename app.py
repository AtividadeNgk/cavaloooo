...
async def registro_processar_novo_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message and update.message.text:
        ...
        processing_msg = await update.message.reply_text(
            ...
            "Por favor, aguarde enquanto transferimos todas as configurações.",
            parse_mode='HTML'
        )
        ...
        try:
            ...
            # 3. Copia todas as configurações
            bot_antigo = manager.get_bot_by_id(bot_id_antigo)
            if bot_antigo:
                # Cria o novo bot com TODAS as configurações do antigo
                manager.create_bot(
                    id=new_bot_id,
                    token=new_token,
                    owner=str(user_id),
                    config=json.loads(bot_antigo[3]),  # config
                    admin=json.loads(bot_antigo[4]),   # admin
                    plans=json.loads(bot_antigo[5]),   # plans
                    gateway=json.loads(bot_antigo[6]), # gateway
                    users=json.loads(bot_antigo[7]),   # users
                    upsell=json.loads(bot_antigo[8]),  # upsell
                    group=bot_antigo[9],               # group
                    expiration=json.loads(bot_antigo[10]) # expiration
                )
                
                # Copia configurações adicionais se existirem
                try:
                    # Copia order bumps
                    orderbumps = manager.get_bot_orderbump(bot_id_antigo)
                    if orderbumps:
                        manager.update_bot_orderbump(new_bot_id, orderbumps)
                    
                    # Copia downsell
                    downsell = manager.get_bot_downsell(bot_id_antigo)
                    if downsell:
                        manager.update_bot_downsell(new_bot_id, downsell)
                    
                    # Copia recuperações
                    recoveries = manager.get_bot_recovery(bot_id_antigo)
                    if recoveries:
                        manager.update_bot_recovery(new_bot_id, recoveries)
                    
                    # Copia disparos programados
                    broadcasts = manager.get_bot_scheduled_broadcasts(bot_id_antigo)
                    if broadcasts:
                        manager.update_bot_scheduled_broadcasts(new_bot_id, broadcasts)
                except:
                    pass
                ...
                await processing_msg.edit_text(
                    ...
                    f"✨ Todas as configurações foram transferidas!\n"
